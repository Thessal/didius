...
def calculate_stat(position_raw, position, x_logret):
    ...
    stat = {
        "min_coverage": np.minimum.accumulate(np.mean(np.isfinite(position_raw))),
        "returns": np.nanmean(returns)*252,
        "sharpe": np.nanmean(returns)/np.nanstd(returns)*np.sqrt(252),
        # "max_turnover": np.maximum.accumulate(np.nanmean(np.abs(np.diff(position)), axis=1)),
        # "mdd": np.min(np.cummax(np.cumsum(returns)) - np.cumsum(returns)),
